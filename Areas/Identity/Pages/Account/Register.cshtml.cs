...
namespace Restaurant.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUserStore<BaseUserModel> _userStore;
        private readonly IUserEmailStore<BaseUserModel> _emailStore;
        private readonly ILogger<RegisterModel> _logger;
        private readonly IEmailSender _emailSender;

        public RegisterModel(
            UserManager<BaseUserModel> userManager,
            IUserStore<BaseUserModel> userStore,
        ...
        private CreateUserModel CreateUser()
        {
            ...
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(CreateUserModel)}'. " +
                    $"Ensure that '{nameof(CreateUserModel)}' is not an abstract class and has a parameterless constructor, or alternatively " +
                    $"override the register page in /Areas/Identity/Pages/Account/Register.cshtml");
            }
        }

        private IUserEmailStore<BaseUserModel> GetEmailStore()
        {
            if (!_userManager.SupportsUserEmail)
            {
                throw new NotSupportedException("The default UI requires a user store with email support.");
            }
            return (IUserEmailStore<BaseUserModel>)_userStore;
        }
    }
}
